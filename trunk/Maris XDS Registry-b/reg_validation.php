		...
		if($ExtrinsicObject_child_node_tagname=='Slot')
		{
			...
			if($slot_name=='hash')
			{
				...
				if($slot_child_node_tagname=='ValueList')
				{
					...
					## UN SOLO VALUE
					  $value_node = $valuelist_child_nodes[0];
...
function validate_Replacement($dom,$DocumentEntryPatientId_array,$connessione)
{
	...
	for($i=0;$i<count($dom_ebXML_RegistryObjectList_child_nodes);$i++)
	{
		...
		if($dom_ebXML_RegistryObjectList_child_node_tagname=='ExtrinsicObject')
		{
			#### SINGOLO NODO EXTRINSICOBJECT
			$ExtrinsicObject_node = $dom_ebXML_RegistryObjectList_child_node;

			$ExtrinsicObject_node_id_attr=$ExtrinsicObject_node->get_attribute('id');

			$ExtrinsicObject_node_id_attr_arr[$ExtrinsicObject_node_id_attr]=$ExtrinsicObject_node_id_attr;

		}//END OF if($dom_ebXML_RegistryObjectList_child_node_tagname=='ExtrinsicObject')
...
if($performControls)
{
	...
	if($isEmpty_7)
	{
		...
		$errorcode[]="XDSRegistryMetadataError";
		$failure_message[]="Association: targetObject ($Association_node_targetObject_attr) is not present in the Registry or you try to append a document to a deprecated document";
...
##### CONTROLLO DELLA PRESENZA NELL'EBXML SI HASH + SIZE + repositoryUniqueId
##### IN CASO CONTRARIO RESTITUISCE ERROR IN VALIDATION
function arePresent_HASH_SIZE_repositoryUniqueId($dom)
{
	##### RADICE DEL DOCUMENTO ebXML
	$root_ebXML = $dom->document_element();
	...
	$size_bool = true;
	$error_on_size="";
	$repositoryUniqueId_bool = true;
	$error_on_repositoryUniqueId="";
...
// 				$error_on_size=$error_on_size."$error SIZE   VALUE FOR DOCUMENT '$ExtrinsicObject_node_id_attr'\n";
			...
			if(strtoupper($attribute) != "REPOSITORYUNIQUEID")
			{
				$repositoryUniqueId_bool = (false || $repositoryUniqueId_bool);
// 				$error_on_repositoryUniqueId=$error_on_repositoryUniqueId."$error repositoryUniqueId   VALUE FOR DOCUMENT '$ExtrinsicObject_node_id_attr'\n";
			}
			else if(strtoupper($attribute) == "REPOSITORYUNIQUEID"){

				$repositoryUniqueId_bool = ($repositoryUniqueId_bool || true);
				$error_on_repositoryUniqueId_1="";
			     }
					}//END OF if($name == "Slot")
			...
			if(!$repositoryUniqueId_bool)
			{
			
			$error_on_repositoryUniqueId.="Error";
			$errorcode[]="XDSRegistryMetadataError";
			$failure_hsu[]=$error_on_repositoryUniqueId."$error -- repositoryUniqueId --    VALUE FOR DOCUMENT '$ExtrinsicObject_node_id_attr'";
			}
...
############# SETTO LE VARIABILI BOOLEANE
	if($error_on_hash!="") $hash_bool=false;
	if($error_on_size!="") $size_bool=false;
	if($error_on_repositoryUniqueId!="") $repositoryUniqueId_bool=false;
	
####ARRAY DI RITORNO
	/*$ret = array($hash_bool,$error_on_hash,$size_bool,$error_on_size,$repositoryUniqueId_bool,$error_on_repositoryUniqueId);

	return $ret;*/

	$valid = $hash_bool && $size_bool && $repositoryUniqueId_bool;
	$ret = array($valid,$failure_hsu,$errorcode);

	return $ret;

}//END OF arePresent_HASH_SIZE_repositoryUniqueId($dom)
...
function controllaQuery($SQLQuery)
{
	...
	if(!($pos_1===0) || $pos_2) // || $pos_3) // || $pos_4)
	{
		...
		$file_input=$_SESSION['tmpQueryService_path'].$_SESSION['idfile']."-query_not_allowed-".$_SESSION['idfile'];
		$fp = fopen($file_input, "wb+");
            		fwrite($fp,$query_not_allowed_response);
         	fclose($fp);
